...
{
  os::fill();
  os::text(F("A R D U I N O   T E T R I S"), 6, 10, LTCYAN);
  os::text(F("Press  SPACE"), 13, 23, LTYELLOW);
}

void drawfield()
{
  os::fill();
  for (byte i=0; i<5; i++) os::text(F("<!..........!>"), 12, i, LTCYAN);
  for (byte i=5; i<20; i++) os::text(F("<!..........!>"), 12, i, WHITE);
  os::text(F("<!==========!>"), 12, 20, LTCYAN);
  os::text(F("  VVVVVVVVVV  "), 12, 21, LTMAGENTA);
  os::text(F("SCORE 0"), 0, 0, GREEN);
  os::text(F("HIGH"), 27, 0);    
  os::text(String(high, DEC), 32, 0);
  os::text(F("CONTROLS"), 0, 3, LTCYAN);
  os::text(F("A - Left"), 0, 5, LTGRAY);
  os::text(F("D - Right"), 0, 6, LTGRAY);
  os::text(F("W - Rotate"), 0, 7, LTGRAY);
  os::text(F("S - Drop"), 0, 8, LTGRAY);
}
